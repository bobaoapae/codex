...
| Operations Dock / Task Board | Complete for read-only plans: fixed below the composer/status area, responsive 80–160 column layout, scrolling, persisted latest plan, resumed snapshot, and keyboard focus. Mouse is optional and disabled in the local configuration. | `tui/src/operations_dock`, `tui/src/app.rs` |
| Agent panel | Uses stable `AgentNavigationState` ordering. Rows prefer the Codex nickname/role and fall back to the canonical task-oriented `agent_path`, avoiding indistinguishable `Agent` labels when v2 metadata is incomplete. From the newest lower composer boundary, `Down` focuses Agents; `F6` remains the fallback. The active thread is marked in the dock; Main is always the first row. Enter opens a thread, `m` returns to Main, and uppercase `I` interrupts only the selected subagent through the existing app-server turn flow. Opening a thread returns focus to the composer; completed agents remain visible. | `tui/src/operations_dock`, `tui/src/app/input.rs` |
