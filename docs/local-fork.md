...
| Agent panel | Uses stable `AgentNavigationState` ordering. Enter opens an agent thread and `i` interrupts it through the existing app-server turn flow; completed agents remain visible. | `tui/src/operations_dock`, `tui/src/app/input.rs` |
