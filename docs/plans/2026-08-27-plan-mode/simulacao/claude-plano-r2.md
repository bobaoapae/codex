...
**Confirmadas pelo utilizador (27/08, via perguntas):**

| # | Decisão | Alternativa rejeitada |
|---|---|---|
| D2 | **Um ficheiro por thread**: `~/.codex/plans/<YYYY-MM-DDTHH-MM-SS>-<slug>.md`, nome fixado no 1.º plano; planos seguintes na mesma thread reescrevem o ficheiro (`revision++`, `updated_at`), corpo idêntico ⇒ não escreve (o template manda repetir o plano em follow-ups de esclarecimento). É o modelo do plan file do Claude. | Um ficheiro por bloco (lista com repetidos); por thread + revisões arquivadas em `plans/history/`. |
| D5 | App-server: **`plan/list` + `plan/read`** apenas. Carregar = o cliente compõe `turn/start` com o template de prompt documentado no README. | `plan/load` server-side (duplicaria `turn/start`); `handoffPrompt` pré-montado no `plan/read`. |
| D7 | Injeção de contexto no TUI **igual ao IDE context**: prefixo + delimitador `## My request for Codex:` (`tui/src/ide_context/prompt.rs:16`); todas as superfícies (TUI, Desktop, export) mostram só o que vem depois do delimitador. Duas ações: **«Implement this plan»** e **«Attach to my next message»**. | Plano visível como mensagem do user (como «Yes, clear context and implement»); só «Implement»; ação extra «Revise in Plan mode». |
| D8 | Sem feature flag e sem opção de config: gravação **sempre ligada** (como os rollouts). | `[plans] enabled` no `config.toml` (toca `ConfigToml` + schema); `Feature::Plans` (toca `features/src/lib.rs`, conflitos de sync). |

**Técnicas (derivadas do código; alternativa em 1 linha):**
...
**Assunções secundárias (cosméticas/técnicas; mudam-se sem impacto no desenho):**
S1 nome `YYYY-MM-DDTHH-MM-SS-<slug>.md` (padrão de memories/rollouts; hora local) · S2 título = 1.º heading `#` → 1.ª linha
não vazia → «Plan <data>» · S4 grava em **todas** as sessões, incl. `exec` e sub-agentes em Plan mode · S7 picker com tabs
«This project»/«All» (default «This project» se houver planos do cwd) · S8 sem linha «Plan saved» em replay/resume ·
S9 métodos **não** experimentais (palpite: o Desktop pode não ligar `experimentalApi`) · S10 sem row «Cancel» no popup
(Esc volta ao picker) · S11 sem cap de tamanho na injeção (planos reais: 0,8–4k palavras) · «Implement» envia a mesma
mensagem do popup existente («Implement the plan.») em Default mode.
