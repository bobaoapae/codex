...
followup = (
    open(followup_file, encoding="utf-8").read().strip() if followup_file else None
)
...
PLANS_DIR = os.path.join(
    os.environ.get("CLAUDE_CONFIG_DIR", os.path.expanduser("~/.claude")), "plans"
)
...
def handle(msg):
        ...
        if tool == "AskUserQuestion":
            ...
            for qd in inp.get("questions", []):
                ...
                summary["questions"].append(
                    {
                        "round": len(summary["rounds"]),
                        "header": qd.get("header"),
                        "question": qd.get("question"),
                        "options": opts,
                        "multiSelect": qd.get("multiSelect"),
                        "picked": pick,
                        "t": round(time.time() - summary["started_at"], 1),
                    }
                )
            ...
            if followup and summary["exit_plan_calls"] == 1:
                ...
                control_reply(
                    ...
                    {
                        "behavior": "deny",
                        "message": "Plano recebido. Fim da sessão de simulação — não implementes nada.",
                        "interrupt": True,
                    },
                )
            ...
            if os.path.normpath(PLANS_DIR).lower() in os.path.normpath(path).lower():
                ...
                control_reply(
                    ...
                    {
                        "behavior": "deny",
                        "message": "Plan mode: only the plan file may be written.",
                        "interrupt": False,
                    },
                )
        else:
    ...
    if current is None:
        return
    if t == "assistant":
        current["last_result_t"] = (
            None  # the model woke up again (e.g. background task finished)
        )
        for block in (msg.get("message") or {}).get("content", []):
            if block.get("type") == "text" and block.get("text"):
                ...
                if name in ("Agent", "Task"):
                    current["subagents"] += 1
                if name in ("Write", "Edit") and (block.get("input") or {}).get(
                    "file_path"
                ):
...
try:
    ...
    while not stop:
        if time.time() - start > TOTAL_TIMEOUT or time.time() - last > IDLE_TIMEOUT:
            ...
            break
        if (
            current.get("last_result_t")
            and time.time() - current["last_result_t"] > RESULT_IDLE
            and summary["exit_plan_calls"] == 0
        ):
            current["errors"].append("ended_without_exit_plan_mode")
            break
        try:
            msg = q.get(timeout=5)
        except queue.Empty:
            continue
        if msg is None:
            current["errors"].append("claude exited")
            break
        last = time.time()
        L("recv", msg)
        handle(msg)
    ...
    try:
        ...
        proc.kill()
print(
    "DONE rounds=%d questions=%d exit_plan_calls=%d"
    % (len(summary["rounds"]), len(summary["questions"]), summary["exit_plan_calls"])
)
