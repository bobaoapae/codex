...
def handle_async(msg):
    ...
    if tid and summary.get("thread_id") and tid != summary["thread_id"]:
        if method == "item/completed":
            ...
            current["subagent_turns_completed"] = (
                current.get("subagent_turns_completed", 0) + 1
            )
        elif method == "thread/tokenUsage/updated":
            current["subagent_token_usage"] = current.get("subagent_token_usage", {})
            current["subagent_token_usage"][tid] = (p.get("tokenUsage") or {}).get(
                "total"
            )
        return
    if method == "item/completed":
...
try:
    rid = request(
        "initialize",
        {
            "clientInfo": {"name": "plan_sim", "title": "Plan sim", "version": "0.1"},
