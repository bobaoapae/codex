...
Local rollout compression compresses every cold rollout, shared lineages included. There is no
reader-capability gate: every reader resolves rollouts through the zstd-aware helpers in
`codex-rollout` (`open_rollout_line_reader`, `open_rollout_seekable_reader`), so a compressed
ancestor keeps serving byte offsets recorded in a fork's `history_base`. Compression stays
best-effort and journalled: an interrupted replacement is recovered or reverted on the next run.
