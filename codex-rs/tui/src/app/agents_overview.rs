...
//!
//! With a primary thread selected, `/agents` shows the durable fleet of that root instead;
//! the daemon-wide listing is the fallback for a session that has no root yet.
...
impl App {
    pub(super) fn open_agents_overview(&mut self, app_server: &AppServerSession) {
        ...
        // FORK: a session with a fleet root shows the fork's dashboard instead
        // of upstream's daemon-wide overview.
