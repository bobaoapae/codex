...
fn spawn_agent_tool_v2_requires_task_name_and_lists_visible_models() {
    ...
    let mut luna = model_preset("gpt-5.6-luna", /*show_in_picker*/ true);
    ...
    assert!(description.contains(
        "- `gpt-5.6-luna`: gpt-5.6-luna description Reasoning efforts: medium (default). Service tiers: priority."
    ));
