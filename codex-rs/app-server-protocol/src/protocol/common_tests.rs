...
#[test]
fn fork_invariant_workspace_lease_methods_keep_tokens_out_of_list_data() -> Result<()> {
    assert_eq!(
        serde_json::to_value([v2::WorkspaceLeaseMode::Read, v2::WorkspaceLeaseMode::Write,])?,
        json!(["read", "write"])
    );
    assert_eq!(
        serde_json::to_value([
            v2::WorkspaceLeaseState::Active,
            v2::WorkspaceLeaseState::Released,
            v2::WorkspaceLeaseState::Expired,
            v2::WorkspaceLeaseState::Recoverable,
        ])?,
        json!(["active", "released", "expired", "recoverable"])
    );

    let list = ClientRequest::WorkspaceLeaseList {
        request_id: RequestId::Integer(61),
        params: v2::WorkspaceLeaseListParams {
            root_thread_id: "root-1".to_string(),
            owner_thread_id: Some("owner-1".to_string()),
    ...
    assert_eq!(
        crate::experimental_api::ExperimentalApi::experimental_reason(&list),
        Some("workspaceLease/list")
    );

    let grant = ClientRequest::WorkspaceLeaseGrant {
        request_id: RequestId::Integer(62),
        params: v2::WorkspaceLeaseGrantParams {
            root_thread_id: "root-1".to_string(),
            owner_thread_id: "owner-1".to_string(),
            paths: vec!["/workspace/repo".to_string()],
            mode: v2::WorkspaceLeaseMode::Write,
    ...
    assert_eq!(
        grant.serialization_scope(),
        Some(ClientRequestSerializationScope::Global("workspace-leases"))
    );
    assert_eq!(
        crate::experimental_api::ExperimentalApi::experimental_reason(&grant),
        Some("workspaceLease/grant")
    );

    let lease = v2::WorkspaceLease {
        lease_id: "lease-1".to_string(),
        root_thread_id: "root-1".to_string(),
        owner_thread_id: "owner-1".to_string(),
        normalized_paths: vec!["/workspace/repo".to_string()],
        mode: v2::WorkspaceLeaseMode::Write,
        state: v2::WorkspaceLeaseState::Active,
        generation: 1,
        environment_id: Some("local".to_string()),
    ...
    let list_response = v2::WorkspaceLeaseListResponse {
        data: vec![lease.clone()],
        next_cursor: None,
    };
    let list_json = serde_json::to_value(list_response)?;
    assert!(!list_json.to_string().contains("token"));
    assert!(!list_json.to_string().contains("secret-token"));

    let grant_response = v2::WorkspaceLeaseGrantResponse {
        leases: vec![v2::WorkspaceLeaseGrant {
            lease,
            token: "secret-token".to_string(),
        }],
    };
    assert_eq!(
        serde_json::to_value(grant_response)?,
        json!({
            "leases": [{
                "lease": {
                    "leaseId": "lease-1",
                    "rootThreadId": "root-1",
                    "ownerThreadId": "owner-1",
                    "normalizedPaths": ["/workspace/repo"],
                    "mode": "write",
                    "state": "active",
                    "generation": 1,
                    "environmentId": "local",
    ...
    let release = ClientRequest::WorkspaceLeaseRelease {
        request_id: RequestId::Integer(63),
    ...
    assert_eq!(
        serde_json::to_value(&release)?["method"],
        "workspaceLease/release"
    );
    assert_eq!(
        crate::experimental_api::ExperimentalApi::experimental_reason(&release),
        Some("workspaceLease/release")
    );
    assert!(
        serde_json::from_value::<v2::WorkspaceLeaseReleaseParams>(json!({
            "rootThreadId": "root-1",
            "leaseId": "lease-1",
            "token": "secret-token",
        }))
